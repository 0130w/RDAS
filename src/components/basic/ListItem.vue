  ...
  <div @click="handleClick">
    <article class="flex items-start space-x-6 p-6">
      ...
      <div class="min-w-0 relative flex-auto">
        <h2 class="font-semibold text-slate-900 truncate pr-20">{{ business.name }}</h2>
        <dl class="mt-2 flex flex-wrap text-sm leading-6 font-medium whitespace-normal">
          <div class="absolute top-0 right-0 flex items-center space-x-1">
            ...
            <dd>{{ business.stars }}</dd>
          </div>
          <div>
            <dt class="sr-only">is_open</dt>
            <dd :class="`inline-block px-1.5 ring-1 px-3 ring-slate-200 rounded-lg font-semibold ${business.is_open === 1 ? 'bg-green-200' : 'bg-gray-300'}`">
              {{ business.is_open === 1 ? 'Opening' : 'Closed' }}
            </dd>
          </div>
          <div class="ml-2">
...
export default {
  ...
  methods: {
    handleClick() {
      // 点击ListItem时将business.business_id传递给父组件
      this.$emit('select-business', this.business.business_id);
    },
  },
};
</script>
